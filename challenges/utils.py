...
def axan_to_rot_matrix(data):
    ...
    rot = np.array(quaternion.as_rotation_matrix(qs)) # size Batch x 24 x 3 x 3

    # finally we get Batch X 24*3*3
    rot = np.reshape(rot, (batch, n*3*3))
    return rot
...
def subtract_flat_id(rot_mats, hands=False):
    ...
    Returns:
        Dec16, modified by Xianghui, add SMPL-H supports
    ...
    id_flat = np.tile(id_flat, (rot_mats.shape[0], J))
