...
def mssd(R_est, t_est, R_gt, t_gt, pts, syms):
  ...
  original: Rp + t
  after symmetry: R(R_s p + t_s) + t remains the same
  """
    ...
    for sym in syms:
        ...
        pts_gt_sym = misc.transform_pts_Rt(pts, R_gt_sym, t_gt_sym)  # definition of R_sym, t_sym: Rp + t
        es.append(np.linalg.norm(pts_est - pts_gt_sym, axis=1).max())
    return min(es)


def mssd_verts(pts_est, R_gt, t_gt, pts, syms):
    """
    Maximum symmetry-aware suface distance, with precomputed estimated object verts
    :param pts_est: Nx3, estimated object verts
    :param R_gt: 3x3 ndarray of the gt rotation
    :param t_gt: 3x1 ndarray of the gt translation
    :param pts: template object verts
    :param syms: a list of symmetries, each is a dict of R, t
    :return:
    """
    es = []
    for sym in syms:
        R_gt_sym = np.matmul(R_gt, np.array(sym['R']))
        t_gt_sym = np.matmul(R_gt, np.array(sym['t']).reshape((3, 1))) + t_gt.reshape((3, 1))
        pts_gt_sym = misc.transform_pts_Rt(pts, R_gt_sym, t_gt_sym)
        es.append(np.linalg.norm(pts_est - pts_gt_sym, axis=1).max()) # compute the max distance
    return min(es) # return min distance of all symmetries
...
def mspd_verts(pts_est, R_gt, t_gt, K, pts, syms):
    """
    Maximum Symmetry-Aware Projection Distance (MSPD).
    :param pts_est: Nx3, estimated object verts
    :param R_gt: 3x3 ndarray of the gt rotation
    ...
    :param K: 3x3 ndarray with the intrinsic camera matrix.
    :param syms: a list of symmetries, each is a dict of R, t
    :return: MSPD error in pixel space
    """
    proj_est = misc.project_pts(pts_est, K, np.eye(3), np.zeros((3, 1)))
    es = []
    for sym in syms:
      R_gt_sym = R_gt.dot(np.array(sym['R']))
      t_gt_sym = R_gt.dot(np.array(sym['t']).reshape((3, 1))) + t_gt.reshape((3, 1))
      proj_gt_sym = misc.project_pts(pts, K, R_gt_sym, t_gt_sym)
      es.append(np.linalg.norm(proj_est - proj_gt_sym, axis=1).max())
    return min(es)
...
def re_symm(R_est, R_gt, syms):
    """
    symmetry aware rotation error
    :param R_est:
    :param R_gt:
    :param syms: a list of symmetry transformations
    :return:
    """
    res = []
    for sym in syms:
        R_gt_sym = np.matmul(R_gt, np.array(sym['R']))
        err = re(R_est, R_gt_sym)
        res.append(err)
    return min(res)
